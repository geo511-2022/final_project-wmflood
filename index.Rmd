...
idx_hNDVI = raster::raster(file_var[2]) 
```
...
days_fire_proj = projectRaster(days_fire, crs = crs(idx_hNDVI))

#Creating an extent to crop to. This is the extent of the idx_hNDVI data set since it is the smallest data set.
e = extent(idx_hNDVI)
...
vegmap_proj = st_transform(vegmap, crs = crs(idx_hNDVI))
...
mapview(Fynbos_sf)
```
Clipping the fire_crop and the idx_hNDVI to only the regions of the Fynbos_sp.
```{r}
#fynbos_geom = Fynbos_sf %>%
#  dplyr::select(geometry) %>%
#  as("Spatial")
#fynbos_e = extent(bbox(fynbos_geom))

#fire_fynbos = crop(fire_crop, fynbos_e)

hNDVI_fynbos = st_crop(idx_hNDVI, Fynbos_sf)

#cropped to  the extent of the file not the shape
```
