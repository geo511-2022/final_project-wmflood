...
```{r}
#fire_fynbos_sf = st_as_sf(fire_fynbos, as_points = TRUE, na.rm = TRUE)

hndvi_fire = st_join(hNDVI_fynbos, fire_fynbos_sf, join = st_intersects, what = "left1", as_points = TRUE)
```
Coercing the stars object to a raster object. This is being done for several reasons. The first is that stars objects were easier to use and manipulate early on with the data sets. Doing the same with raster objects would have been more complicated and time consuming. The second reason to convert to a raster object now is that it is easier to make a dataframe for the next step from a raster than a stars object.
```{r}
hndvi_df = as.data.frame(hndvi_fire, center = TRUE)
```
